...
export const appointmentRepositories = [
  ...
  },
  {
    ...
    inject: ['DATABASE_CONNECTION', 'LOGGER_SERVICE'],
    useFactory: (connection: MongoConnection, logger: Logger) => new MongoPatientRepository(connection, logger),
  },
  {
    provide: 'APPOINTMENT_DOCTOR_REPOSITORY',
    inject: ['DATABASE_CONNECTION', 'LOGGER_SERVICE'],
    useFactory: (connection: MongoConnection, logger: Logger) => new MongoDoctorRepository(connection, logger),
  },
];
